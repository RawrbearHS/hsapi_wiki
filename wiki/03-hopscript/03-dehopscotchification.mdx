...
Names will be dehopscotchified in the reverse of what is described in [name hopscotchification](/wiki/hopscript/basic-syntax#name-hopscotchification). If the dehopscotchified name will not be rehopscotchified to the same name, then it will use the string literal form of the name.
...
If the project being dehopscotchified contains a block that does not exactly match one that hopscript knows about, it will include the relevant code to define it. See the section on basic type definitions for more on how.
